

/*
WARNING: THIS FILE IS AUTO-GENERATED. DO NOT MODIFY.

This file was generated from example2.idl using "rtiddsgen".
The rtiddsgen tool is part of the RTI Connext distribution.
For more information, type 'rtiddsgen -help' at a command shell
or consult the RTI Connext manual.
*/

#ifndef example2Plugin_132753538_h
#define example2Plugin_132753538_h

#include "example2.hpp"

struct RTICdrStream;

#ifndef pres_typePlugin_h
#include "pres/pres_typePlugin.h"
#endif

#include "examplePlugin.hpp"

#if (defined(RTI_WIN32) || defined (RTI_WINCE)) && defined(NDDS_USER_DLL_EXPORT)
/* If the code is building on Windows, start exporting symbols.
*/
#undef NDDSUSERDllExport
#define NDDSUSERDllExport __declspec(dllexport)
#endif

#define exampleStructPlugin_get_sample PRESTypePluginDefaultEndpointData_getSample 
#define exampleStructPlugin_get_buffer PRESTypePluginDefaultEndpointData_getBuffer 
#define exampleStructPlugin_return_buffer PRESTypePluginDefaultEndpointData_returnBuffer 

#define exampleStructPlugin_create_sample PRESTypePluginDefaultEndpointData_createSample 
#define exampleStructPlugin_destroy_sample PRESTypePluginDefaultEndpointData_deleteSample 

/* --------------------------------------------------------------------------------------
Support functions:
* -------------------------------------------------------------------------------------- */

NDDSUSERDllExport extern exampleStruct*
exampleStructPluginSupport_create_data_w_params(
    const struct DDS_TypeAllocationParams_t * alloc_params);

NDDSUSERDllExport extern exampleStruct*
exampleStructPluginSupport_create_data_ex(RTIBool allocate_pointers);

NDDSUSERDllExport extern exampleStruct*
exampleStructPluginSupport_create_data(void);

NDDSUSERDllExport extern RTIBool 
exampleStructPluginSupport_copy_data(
    exampleStruct *out,
    const exampleStruct *in);

NDDSUSERDllExport extern void 
exampleStructPluginSupport_destroy_data_w_params(
    exampleStruct *sample,
    const struct DDS_TypeDeallocationParams_t * dealloc_params);

NDDSUSERDllExport extern void 
exampleStructPluginSupport_destroy_data_ex(
    exampleStruct *sample,RTIBool deallocate_pointers);

NDDSUSERDllExport extern void 
exampleStructPluginSupport_destroy_data(
    exampleStruct *sample);

NDDSUSERDllExport extern void 
exampleStructPluginSupport_print_data(
    const exampleStruct *sample,
    const char *desc,
    unsigned int indent);

/* ----------------------------------------------------------------------------
Callback functions:
* ---------------------------------------------------------------------------- */

NDDSUSERDllExport extern PRESTypePluginParticipantData 
exampleStructPlugin_on_participant_attached(
    void *registration_data, 
    const struct PRESTypePluginParticipantInfo *participant_info,
    RTIBool top_level_registration, 
    void *container_plugin_context,
    RTICdrTypeCode *typeCode);

NDDSUSERDllExport extern void 
exampleStructPlugin_on_participant_detached(
    PRESTypePluginParticipantData participant_data);

NDDSUSERDllExport extern PRESTypePluginEndpointData 
exampleStructPlugin_on_endpoint_attached(
    PRESTypePluginParticipantData participant_data,
    const struct PRESTypePluginEndpointInfo *endpoint_info,
    RTIBool top_level_registration, 
    void *container_plugin_context);

NDDSUSERDllExport extern void 
exampleStructPlugin_on_endpoint_detached(
    PRESTypePluginEndpointData endpoint_data);

NDDSUSERDllExport extern void    
exampleStructPlugin_return_sample(
    PRESTypePluginEndpointData endpoint_data,
    exampleStruct *sample,
    void *handle);    

NDDSUSERDllExport extern RTIBool 
exampleStructPlugin_copy_sample(
    PRESTypePluginEndpointData endpoint_data,
    exampleStruct *out,
    const exampleStruct *in);

/* ----------------------------------------------------------------------------
(De)Serialize functions:
* ------------------------------------------------------------------------- */

NDDSUSERDllExport extern RTIBool 
exampleStructPlugin_serialize(
    PRESTypePluginEndpointData endpoint_data,
    const exampleStruct *sample,
    struct RTICdrStream *stream, 
    RTIBool serialize_encapsulation,
    RTIEncapsulationId encapsulation_id,
    RTIBool serialize_sample, 
    void *endpoint_plugin_qos);

NDDSUSERDllExport extern RTIBool 
exampleStructPlugin_deserialize_sample(
    PRESTypePluginEndpointData endpoint_data,
    exampleStruct *sample, 
    struct RTICdrStream *stream,
    RTIBool deserialize_encapsulation,
    RTIBool deserialize_sample, 
    void *endpoint_plugin_qos);

NDDSUSERDllExport extern RTIBool
exampleStructPlugin_serialize_to_cdr_buffer(
    char * buffer,
    unsigned int * length,
    const exampleStruct *sample); 

NDDSUSERDllExport extern RTIBool 
exampleStructPlugin_deserialize(
    PRESTypePluginEndpointData endpoint_data,
    exampleStruct **sample, 
    RTIBool * drop_sample,
    struct RTICdrStream *stream,
    RTIBool deserialize_encapsulation,
    RTIBool deserialize_sample, 
    void *endpoint_plugin_qos);

NDDSUSERDllExport extern RTIBool
exampleStructPlugin_deserialize_from_cdr_buffer(
    exampleStruct *sample,
    const char * buffer,
    unsigned int length);    

NDDSUSERDllExport extern RTIBool
exampleStructPlugin_skip(
    PRESTypePluginEndpointData endpoint_data,
    struct RTICdrStream *stream, 
    RTIBool skip_encapsulation,  
    RTIBool skip_sample, 
    void *endpoint_plugin_qos);

NDDSUSERDllExport extern unsigned int 
exampleStructPlugin_get_serialized_sample_max_size_ex(
    PRESTypePluginEndpointData endpoint_data,
    RTIBool * overflow,
    RTIBool include_encapsulation,
    RTIEncapsulationId encapsulation_id,
    unsigned int current_alignment);    

NDDSUSERDllExport extern unsigned int 
exampleStructPlugin_get_serialized_sample_max_size(
    PRESTypePluginEndpointData endpoint_data,
    RTIBool include_encapsulation,
    RTIEncapsulationId encapsulation_id,
    unsigned int current_alignment);

NDDSUSERDllExport extern unsigned int 
exampleStructPlugin_get_serialized_sample_min_size(
    PRESTypePluginEndpointData endpoint_data,
    RTIBool include_encapsulation,
    RTIEncapsulationId encapsulation_id,
    unsigned int current_alignment);

NDDSUSERDllExport extern unsigned int
exampleStructPlugin_get_serialized_sample_size(
    PRESTypePluginEndpointData endpoint_data,
    RTIBool include_encapsulation,
    RTIEncapsulationId encapsulation_id,
    unsigned int current_alignment,
    const exampleStruct * sample);

/* --------------------------------------------------------------------------------------
Key Management functions:
* -------------------------------------------------------------------------------------- */
NDDSUSERDllExport extern PRESTypePluginKeyKind 
exampleStructPlugin_get_key_kind(void);

NDDSUSERDllExport extern unsigned int 
exampleStructPlugin_get_serialized_key_max_size_ex(
    PRESTypePluginEndpointData endpoint_data,
    RTIBool * overflow,
    RTIBool include_encapsulation,
    RTIEncapsulationId encapsulation_id,
    unsigned int current_alignment);

NDDSUSERDllExport extern unsigned int 
exampleStructPlugin_get_serialized_key_max_size(
    PRESTypePluginEndpointData endpoint_data,
    RTIBool include_encapsulation,
    RTIEncapsulationId encapsulation_id,
    unsigned int current_alignment);

NDDSUSERDllExport extern RTIBool 
exampleStructPlugin_serialize_key(
    PRESTypePluginEndpointData endpoint_data,
    const exampleStruct *sample,
    struct RTICdrStream *stream,
    RTIBool serialize_encapsulation,
    RTIEncapsulationId encapsulation_id,
    RTIBool serialize_key,
    void *endpoint_plugin_qos);

NDDSUSERDllExport extern RTIBool 
exampleStructPlugin_deserialize_key_sample(
    PRESTypePluginEndpointData endpoint_data,
    exampleStruct * sample,
    struct RTICdrStream *stream,
    RTIBool deserialize_encapsulation,
    RTIBool deserialize_key,
    void *endpoint_plugin_qos);

NDDSUSERDllExport extern RTIBool 
exampleStructPlugin_deserialize_key(
    PRESTypePluginEndpointData endpoint_data,
    exampleStruct ** sample,
    RTIBool * drop_sample,
    struct RTICdrStream *stream,
    RTIBool deserialize_encapsulation,
    RTIBool deserialize_key,
    void *endpoint_plugin_qos);

NDDSUSERDllExport extern RTIBool
exampleStructPlugin_serialized_sample_to_key(
    PRESTypePluginEndpointData endpoint_data,
    exampleStruct *sample,
    struct RTICdrStream *stream, 
    RTIBool deserialize_encapsulation,  
    RTIBool deserialize_key, 
    void *endpoint_plugin_qos);

/* Plugin Functions */
NDDSUSERDllExport extern struct PRESTypePlugin*
exampleStructPlugin_new(void);

NDDSUSERDllExport extern void
exampleStructPlugin_delete(struct PRESTypePlugin *);

#if (defined(RTI_WIN32) || defined (RTI_WINCE)) && defined(NDDS_USER_DLL_EXPORT)
/* If the code is building on Windows, stop exporting symbols.
*/
#undef NDDSUSERDllExport
#define NDDSUSERDllExport
#endif

#endif /* example2Plugin_132753538_h */

